...
while {_TOF < 15 && (_bulletPos select 1) < _targetRange} do
{
	...
	if (_storeRangeCardData) then {
		_range = ATragMX_rangeCardStartRange + _n * ATragMX_rangeCardIncrement;
		if ((_bulletPos select 1) * _rangeFactor >= _range && _range <= ATragMX_rangeCardEndRange) then {
			...
			ATragMX_rangeCardData set [_n, [_range, _elevation * 60, _windage * 60, _lead, _TOF, _bulletSpeed, _kineticEnergy]];
			_n = _n + 1;
		};
	};
};
