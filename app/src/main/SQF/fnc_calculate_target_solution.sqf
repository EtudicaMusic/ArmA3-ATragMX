...
_targetRange = (ATragMX_targetRange select ATragMX_currentTarget);
...
_result = [_scopeBaseAngle, _bulletMass, _boreHeight, _airFriction, _muzzleVelocity, _temperature, _barometricPressure, _relativeHumidity, 1000,
			_windSpeed, _windDirection, _inclinationAngle, _targetSpeed, _targetRange, _bc, _dragModel, _atmosphereModel, false] call ATragMX_calcSolution;

ATragMX_elevationOutput set [ATragMX_currentTarget, _result select 0];
ATragMX_windageOutput set [ATragMX_currentTarget, _result select 1];
ATragMX_leadOutput set [ATragMX_currentTarget, _result select 2];
ATragMX_tofOutput set [ATragMX_currentTarget, _result select 3];
ATragMX_velocityOutput set [ATragMX_currentTarget, _result select 4];
